...
export default function ShareModal() {
	...
	function createShareUrl(socialType, sharedUrl, sharedText, sharedImage, sharedTitle, sharedCaption) {
		...
		switch (socialType) {
			case "facebook":
				socialShareUrl =
					"https://www.facebook.com/sharer/sharer.php?u=" + sharedUrl + "&title=" + sharedTitle + "&caption=" + sharedCaption + "&description=" + sharedText + "&picture=" + sharedImage
				window.open(socialShareUrl, "Facebook", "width=550,height=400,menubar=1,status=0,toolbar=0,scrollbars=1")
				break

			case "twitter":
				socialShareUrl = "https://twitter.com/intent/tweet?text=" + sharedText + "&url=" + sharedUrl
				window.open(socialShareUrl, "Twitter", "width=550,height=400,menubar=1,status=0,toolbar=0,scrollbars=1")
				break

			case "whatsapp":
				socialShareUrl = "https://api.whatsapp.com/send?text=" + shareText + " " + shareUrl
				window.open(socialShareUrl, "WhatsApp", "width=550,height=400,menubar=1,status=0,toolbar=0,scrollbars=1")
				break

			case "telegram":
				socialShareUrl = "https://t.me/share/url?url=" + shareText + " " + shareUrl
				window.open(socialShareUrl, "Telegram", "width=550,height=400,menubar=1,status=0,toolbar=0,scrollbars=1")
				break

			case "pinterest":
				socialShareUrl = "https://pinterest.com/pin/create/button/?url=" + sharedUrl + "&media=" + sharedImage + "&description=" + sharedText
				window.open(socialShareUrl, "Pinterest", "width=550,height=400,menubar=1,status=0,toolbar=0,scrollbars=1")
				break

			case "linkedin":
				socialShareUrl = "https://www.linkedin.com/shareArticle?mini=true&url=" + sharedUrl + "&title=" + sharedTitle + "&summary=" + sharedText + "&source=" + sharedCaption
	...
	return (
		<>
			<a
				...
				className="rounded-full border border-governor-bay-50 text-governor-bay-50 hover:bg-governor-bay-700 hover:text-white py-2 px-2 md:px-4 inline-block ease-linear transition-all duration-150"
			>
				<ShareIcon className="md:mr-1 h-6 w-6 md:h-4 md:w-4 inline align-middle md:-mt-1" /> <span className="hidden md:inline-block">Condividi</span>
			</a>

			{showModal ? (
				<>
					<div className="justify-center flex overflow-x-hidden overflow-y-auto fixed items-center inset-0 z-50 outline-none focus:outline-none mx-0 ml-0 px-0">
						<div className="relative w-full my-6 mx-auto max-w-[95%] lg:max-w-4xl">
							{/*content*/}
							<div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
								{/*header*/}
								<div className="flex items-start justify-between p-4 border-b border-solid border-gray-200 rounded-t">
									<h3 className="text-2xl font-bold">Ti piace questo sito? Condividilo!</h3>
									<button
								...
								<div className="relative p-6 flex-auto">
									<div className="grid grid-cols-12 gap-3 mt-3">
										<div className="col-span-12 lg:col-span-4">
											<span className="block w-full mb-5 text-xs uppercase font-semibold text-gray-500 tracking-widest">CONDIVIDI SUI SOCIAL</span>

											<a
												href="#"
												onClick={(e) => {
													e.preventDefault()
													createShareUrl("facebook", shareUrl, "", "", "", "")
												}}
											>
												<FaFacebook className="shares-facebook text-4xl lg:text-5xl mr-3 inline" />
											</a>
											<a
												href="#"
												onClick={(e) => {
													e.preventDefault()
													createShareUrl("twitter", shareUrl, shareText, "", "", "")
												}}
											>
												<FaXTwitter className="shares-twitter text-4xl lg:text-5xl mr-3 inline" />
											</a>
											<a
												href="#"
												onClick={(e) => {
													e.preventDefault()
													createShareUrl("whatsapp", shareUrl, shareText, "", "", "")
												}}
											>
												<FaWhatsapp className="shares-whatsapp text-4xl lg:text-5xl mr-3 inline" />
											</a>
											<a
												href="#"
												onClick={(e) => {
													e.preventDefault()
													createShareUrl("telegram", shareUrl, shareText, "", "", "")
												}}
											>
												<FaTelegram className="shares-telegram text-4xl lg:text-5xl mr-3 inline" />
											</a>
										</div>
