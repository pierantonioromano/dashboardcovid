...
export const extractWeeklyData = (inputArray, cleanedDailyData, backDays) => {
	const retVal = inputArray
		.map((item, index) => {
			...
			//Dead people
			const deadDayBefore = inputArray[index - 1]?.deceduti || cleanedDailyData[cleanedDailyData.length - backDays]?.deceduti
			const deadDay = item.deceduti

			//Swabs
			const swabsDayBefore = inputArray[index - 1]?.tamponi || cleanedDailyData[cleanedDailyData.length - backDays]?.tamponi
			const swabsDay = item.tamponi

			//Hospitalized
			const hospitalizedDayBefore = inputArray[index - 1]?.ricoverati_con_sintomi || cleanedDailyData[cleanedDailyData.length - backDays]?.ricoverati_con_sintomi
			const hospitalizedDay = item.ricoverati_con_sintomi

			//ICU
			const icuDayBefore = inputArray[index - 1]?.terapia_intensiva || cleanedDailyData[cleanedDailyData.length - backDays]?.terapia_intensiva
		...
		.reduce((accumulator, item) => {
			Object.keys(item).forEach((key) => {
				if (!isNaN(item[key]) && item[key] !== null) {
					accumulator[key] = parseInt((accumulator[key] || 0) + item[key])
				}
			})
	...
	const graph_trending_series = [
		{
			name: "Nuovi casi",
			type: "area",
			data: cleanedDailyData ? cleanedDailyData.map((item) => [new Date(item.data).getTime(), item.nuovi_positivi]) : null
		},
		{
			name: "T.I. totali",
			type: "area",
			data: cleanedDailyData ? cleanedDailyData.map((item) => [new Date(item.data).getTime(), item.terapia_intensiva]) : null
		},
		{
			name: "Ricoveri totali",
			type: "area",
			data: cleanedDailyData ? cleanedDailyData.map((item) => [new Date(item.data).getTime(), item.totale_ospedalizzati]) : null
		}
	]
